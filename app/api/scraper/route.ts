...
export async function GET(request: Request) {
  ...
  const insertConfiguration = async (config) => {
    const mapConfigToDbColumns = (config) => {
      const mapping = {
        where: "location",
  ...
  async function getServerCacheId(config) {
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();

    await page.setViewport({ width: 1200, height: 800 });

    let serverCacheId = null;
    let offersData = null;
    ...
    try {
      ...
      const offerEntries = [];
      for (const offer of offersData.offers.Electricity.offersList) {
